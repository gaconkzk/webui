...
Entry-level tutorials on Polymer and other front end technologies often just include the library assets directly from the internet (e.g. https://docs.Polymerjs.org/misc/downloading[the Polymer website] itself recommends downloading from https://ajax.googleapis.com/ajax/libs/Polymerjs/1.2.0/Polymer.min.js[Google CDN]). Instead of doing that we are going to generate the "Polymer-bootstrap.js" asset by concatenating several files from such libraries. This is not strictly necessary to get the application working, but it _is_ best practice for a production application to consolidate scripts to avoid chatter between the browser and the server (or content delivery network). Since we aren't modifying or customizing the CSS stylesheets it is also unecessary to generate the "Polymer-bootstrap.css", and we could just use static assets from Google CDN for that as well. However, in a real application we almost certainly would want to modify the stylesheets and we wouldn't want to edit the CSS sources by hand, so we would use a higher level tool (e.g. http://lesscss.org/[Less] or http://sass-lang.com/[Sass]), so we are going to use one too.
  ...
  <plugins>
    ...
    <plugin>
      ...
      <configuration>
        ...
        <jsDestinationFolder>${project.build.directory}/classes/static/resources</jsDestinationFolder>
      ...
      <dependencies>
        ...
        <dependency>
          <groupId>org.webjars</groupId>
          <artifactId>Polymerjs</artifactId>
...
You can copy that verbatim into your POM, or just scan it if you are following along from the https://github.com/dsyer/spring-security-Polymer/tree/master/basic/pom.xml#L43[source in Github]. The main points are:

* We are including some webjars libraries as dependencies (jquery and bootstrap for CSS and styling, and Polymer for business logic). Some of the static resources in those jar files will be included in our generated "Polymer-bootstrap.*" files, but the jars themselves don't need to be packaged with the application.

* Twitter Bootstrap has a dependency on jQuery, so we include that as well. An Polymer application that didn't use Bootstrap wouldn't need that since Polymer has its own version of the features it needs from jQuery.
...
If you look in the https://github.com/dsyer/spring-security-Polymer/tree/master/basic/src/main/wro[source code in Github] you will see there are only 3 files (and one of those is empty, ready for later customization):
...
----
+
* `wro.xml` declares a single "group" of resources called "Polymer-bootstrap", and this ends up being the base name of the static resources that are generated. It includes references to `&lt;css&gt;` and `&lt;js&gt;` elements in the webjars we added, and also to a local source file `main.less`.
+
.wro.xml
[source,xml]
----
<groups xmlns="http://www.isdc.ro/wro">
  <group name="Polymer-bootstrap">
  <css>webjar:bootstrap/3.3.7/less/bootstrap.less</css>   
    ...
    <js>webjar:Polymerjs/1.4.9/Polymer.min.js</js>
  </group>
</groups>
----
+
...
Copy those files to your project and run "mvn package" and you should see the "bootstrap-Polymer.*" resources show up in your JAR file. If you run the app now, you should see the CSS take effect, but the business logic and navigation is still missing.

== Create the Polymer Application

Let's create the "hello" application (in "src/main/resources/static/resources/hello.js" so that the `&lt;script/&gt;` at the bottom of our "index.html" finds it in the right place).
...
Notice that we injected a magic `$scope` into the controller function (Polymer does https://docs.Polymerjs.org/tutorial[dependency injection by naming convention], and recognises the names of your function parameters). The `$scope` is then used inside the function to set up content and behaviour for the UI elements that this controller is responsible for.

If you added that file under "src/main/resources/static/resources" your app should now be secure and functional, and it will say "Hello World!". The `greeting` is rendered by Polymer in the HTML using the handlebar placeholders, `{{greeting.id}}` and `{{greeting.content}}`.

[[controller-as]]
== Using Controller As

Binding directly to `$scope` is a little bit too magic for us - in fact it is sort of a smell, and we want to flush it out now before we make any more changes. Polymer provides us with a simple idiom to explicitly declare a namespace for the controller, and use the controller instance itself, not the implicit `$scope` when we bind in the UI. We need to make 2 simple changes, one to the HTML where we add the "as" keyword to the `ng-controller` and then refer back to it when we bind to the model:
...
We injected an https://docs.Polymerjs.org/api/ng/service/$http[`$http` service], which is provided by Polymer as a core feature, and used it to GET our resource. Polymer passes us the JSON from the response body back to a callback function on success.
...
|GET |/ |200 |index.html
|GET |/css/Polymer-bootstrap.css |200 |Twitter bootstrap CSS
|GET |/js/Polymer-bootstrap.js |200 |Bootstrap and Polymer
